...
public class OnItemUseEvent {
    ...
    private static void onUseItemEffect(ItemStack itemStack, Player player, Level world, int experience) {
    ...
    public static InteractionResultHolder<ItemStack> onUseItem(Player player, Level world, InteractionHand hand) {
        ...
        if (player.experienceLevel <= 50 && CRYSTAL_LIST.contains(itemStack.getItem())) {
            ...
            player.displayClientMessage(Component.nullToEmpty("通过打碎晶体获得的经验等级已达到上限(要求:玩家等级<=50级)"), true);
    ...
    public static InteractionResultHolder<ItemStack> onUseCrystalItem(ItemStack itemStack, Player player, Level world, int experience) {
        // 播放玻璃破碎的声音
    ...
    public static InteractionResultHolder<ItemStack> onUseHayBlockItem(ItemStack itemStack, Player player, Level world, int experience) {
        // 播放干草块破碎的声音
        player.playSound(SoundEvents.GRASS_BREAK, 1.0F, 1.0F);
        //物品使用效果(物品-1)
        onUseItemEffect(itemStack, player, world, experience);
        player.getInventory().placeItemBackInInventory(new ItemStack(Items.WHEAT, 9));
        return InteractionResultHolder.success(itemStack);
