...
public class UpdateArmorEvent {
    ...
    //玩家护甲值下降,套装集齐效果
    public static Component updatePlayerArmor(Player player) {
        ArrayList<ItemStack> armorItemList = new ArrayList<>();
        player.getArmorSlots().forEach(element -> {
        ...
        if (armorItemList.isEmpty()) {
            ...
            return Component.nullToEmpty("The armor equipment is empty");
        }
        ...
        for (ItemStack itemStack : armorItemList) {
            if (itemStack.getItem() instanceof ArmorItem) {
                ...
                //最大护甲值
                int baseProtection = armorItem.getDefense();
                //加到理论最大护甲值里面
                maxProtection = maxProtection + baseProtection;
                //最大耐久
                int baseDurability = itemStack.getMaxDamage();
                //目前耐久
                int durability = baseDurability - itemStack.getDamageValue();
                //满耐久一定获得满护甲值
        ...
        //设定玩家护甲
        player.getAttribute(Attributes.ARMOR).setBaseValue(-protectionReduction);
        //拥有至少10点护甲时,获得抗性提升效果
        if (protection > 10) {
            boolean hasResistance = player.hasEffect(MobEffects.DAMAGE_RESISTANCE);
            if (!hasResistance)
                player.addEffect(new MobEffectInstance(MobEffects.DAMAGE_RESISTANCE, 120, 0, false, false, false));
            else if (player.getEffect(MobEffects.DAMAGE_RESISTANCE).getDuration() <= 20) {
                player.addEffect(new MobEffectInstance(MobEffects.DAMAGE_RESISTANCE, 120, 0, false, false, false));
            }
        }
