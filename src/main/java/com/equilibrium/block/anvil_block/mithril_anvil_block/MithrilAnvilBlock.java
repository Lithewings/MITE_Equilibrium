...
public class MithrilAnvilBlock extends FallingBlock {
    public static final MapCodec<MithrilAnvilBlock> CODEC = createCodec(MithrilAnvilBlock::new);
    ...
    public static final int MITHRIL_ANVIL_MAX_DURABILITY = 512;
    public static final IntProperty MITHRIL_ANVIL_DURABILITY_PROPERTY = IntProperty.of("mithril_anvil_durability",0, MITHRIL_ANVIL_MAX_DURABILITY);
    public static final IntProperty MITHRIL_ANVIL_STAGE = IntProperty.of("mithril_anvil_stage",0,2);


    @Override
    public MapCodec<MithrilAnvilBlock> getCodec() {
        return CODEC;
    }

    public MithrilAnvilBlock(Settings settings) {
        super(settings);
        this.setDefaultState(this.stateManager.getDefaultState()
                .with(FACING, Direction.NORTH)
                .with(MITHRIL_ANVIL_DURABILITY_PROPERTY, MITHRIL_ANVIL_MAX_DURABILITY)
                .with(MITHRIL_ANVIL_STAGE,0));
    }
    ...
    protected List<ItemStack> getDroppedStacks(BlockState state, LootContextParameterSet.Builder builder) {
        ...
        drop.set(DataComponentTypes.MAX_DAMAGE, MITHRIL_ANVIL_MAX_DURABILITY);
        drop.set(DataComponentTypes.DAMAGE, MITHRIL_ANVIL_MAX_DURABILITY - state.get(MITHRIL_ANVIL_DURABILITY_PROPERTY));
        return List.of(drop);
    }
    ...
    public void onPlaced(World world, BlockPos pos, BlockState state, @Nullable LivingEntity placer, ItemStack itemStack) {
        ...
        int durability = MITHRIL_ANVIL_MAX_DURABILITY - itemStack.getDamage();
        int phase = getPhaseFromDurability(MITHRIL_ANVIL_MAX_DURABILITY,durability);
        if (placer != null) {
            placer.sendMessage(Text.of("砧使用状态:"+phase));
            placer.sendMessage(Text.of("砧耐久:"+durability));
        }
        //根据物品耐久,将耐久值放入方块状态中
        world.setBlockState(pos, ModBlocksRegistry.MITHRIL_ANVIL.getDefaultState()
                //copy facing
                .with(MithrilAnvilBlock.FACING, state.get(MithrilAnvilBlock.FACING))
                //copy damage
                .with(MITHRIL_ANVIL_DURABILITY_PROPERTY, durability)
                .with(MITHRIL_ANVIL_STAGE,phase)
        );
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, BlockHitResult hit) {
        ...
        if (world.isClient) {
        ...
        } else if (player.getMainHandStack().isOf(ModBlocksRegistry.MITHRIL_BLOCK.asItem())) {


            //修复
            int durability = state.get(MITHRIL_ANVIL_DURABILITY_PROPERTY);

            int beforePhase = getPhaseFromDurability(MITHRIL_ANVIL_MAX_DURABILITY,durability);

            if (beforePhase==0)
                return ActionResult.PASS;

            //增加耐久
            int fixed = Math.clamp((int)(durability +0.33f*(float) MITHRIL_ANVIL_MAX_DURABILITY), 0, MITHRIL_ANVIL_MAX_DURABILITY);
            ...
            world.setBlockState(pos, ModBlocksRegistry.MITHRIL_ANVIL.getDefaultState()
                    ...
                    .with(MithrilAnvilBlock.FACING, state.get(MithrilAnvilBlock.FACING))
                    //copy damage
                    .with(MITHRIL_ANVIL_DURABILITY_PROPERTY,fixed)
                    .with(MITHRIL_ANVIL_STAGE,afterPhase)
            );
    ...
    protected void appendProperties(StateManager.Builder<Block, BlockState> builder) {
        builder.add(FACING, MITHRIL_ANVIL_DURABILITY_PROPERTY, MITHRIL_ANVIL_STAGE);
    }
    ...
    protected NamedScreenHandlerFactory createScreenHandlerFactory(BlockState state, World world, BlockPos pos) {
        return new SimpleNamedScreenHandlerFactory(
                (syncId, inventory, player) -> new MithrilAnvilScreenHandler(syncId, inventory, ScreenHandlerContext.create(world, pos)), TITLE
        );
    }
