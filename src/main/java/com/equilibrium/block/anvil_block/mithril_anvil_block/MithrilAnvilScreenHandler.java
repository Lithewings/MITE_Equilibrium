...
public class MithrilAnvilScreenHandler extends ForgingScreenHandler {
    ...
    public MithrilAnvilScreenHandler(int syncId, PlayerInventory inventory) {
        this(syncId, inventory, ScreenHandlerContext.EMPTY);
    }

    public MithrilAnvilScreenHandler(int syncId, PlayerInventory inventory, ScreenHandlerContext context) {
    ...
    protected void onTakeOutput(PlayerEntity player, ItemStack stack) {
        ...
        this.context.run((world, pos) -> {
                    ...
                    if (blockState.contains(MITHRIL_ANVIL_DURABILITY_PROPERTY)) {
                        //铁砧目前的耐久
                        int i = blockState.get(MITHRIL_ANVIL_DURABILITY_PROPERTY);
                        //铁砧破坏进度
                        int phase = getPhaseFromDurability(MITHRIL_ANVIL_MAX_DURABILITY,blockState.get(MITHRIL_ANVIL_DURABILITY_PROPERTY));
                        //将耐久-1写入方块状态中,并更新外观状态
                        world.setBlockState(pos,blockState
                                .with(MithrilAnvilBlock.FACING, blockState.get(MithrilAnvilBlock.FACING))
                                .with(MITHRIL_ANVIL_DURABILITY_PROPERTY,Math.clamp(i-1,0, MITHRIL_ANVIL_MAX_DURABILITY))
                                .with(MITHRIL_ANVIL_STAGE,Math.clamp(phase,0,2))
                        );
                    }
                    else
        ...
        this.context.run((world, pos) -> {
            ...
            if (!player.isInCreativeMode()) {
                //耐久为1时,直接损坏
                if(blockState.get(MITHRIL_ANVIL_DURABILITY_PROPERTY)==0) {
